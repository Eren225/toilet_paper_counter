                ...
                <table className="w-full border-collapse text-left">
                  <thead>
                    <tr className="bg-slate-100 dark:bg-slate-900">
                      <th className="border-b border-slate-200 p-6 text-[10px] font-bold uppercase tracking-widest text-slate-600 dark:text-slate-300 dark:border-slate-800">Colocataire</th>
                      <th className="border-b border-slate-200 p-6 text-[10px] font-bold uppercase tracking-widest text-slate-600 dark:text-slate-300 dark:border-slate-800">Ouverts (pack actuel)</th>
                      <th className="border-b border-slate-200 p-6 text-[10px] font-bold uppercase tracking-widest text-slate-600 dark:text-slate-300 dark:border-slate-800">Dernière activité</th>
                      <th className="border-b border-slate-200 p-6 text-right text-[10px] font-bold uppercase tracking-widest text-slate-600 dark:text-slate-300 dark:border-slate-800">Action</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-200 dark:divide-slate-800">
                    {state.roommates.map((roommate) => (
                      <tr className="group transition-colors hover:bg-slate-50 dark:hover:bg-slate-900" key={roommate.id}>
                        <td className="p-6 align-middle">
                          <div className="flex items-center gap-4">
                            <img
                              alt={`Photo de ${roommate.name}`}
                              className="h-8 w-8 rounded-full border border-slate-300 object-cover dark:border-slate-700"
                              src={roommate.avatar}
                            />
                            <div className="h-6 w-1.5 bg-slate-300 transition-colors group-hover:bg-slate-900 dark:bg-slate-700 dark:group-hover:bg-slate-50" />
                            <span className="text-sm font-bold uppercase tracking-tight">{roommate.name}</span>
                          </div>
                        </td>
                        <td className="p-6 align-middle">
                          <span className="font-mono text-sm font-bold text-slate-700 dark:text-slate-200">{roommate.opened}</span>
                        </td>
                        <td className="p-6 align-middle">
                          <span className="font-mono text-xs text-slate-600 dark:text-slate-400">{roommate.lastActive}</span>
                        </td>
                        <td className="p-6 text-right align-middle">
                          <button
                            className="border border-slate-300 bg-transparent px-4 py-2 text-[10px] font-bold uppercase tracking-widest transition-all duration-100 hover:border-slate-900 hover:bg-slate-900 hover:text-white disabled:cursor-not-allowed disabled:opacity-50 dark:border-slate-700 dark:hover:border-slate-50 dark:hover:bg-slate-50 dark:hover:text-slate-900"
                            disabled={roommate.id !== state.auth.currentUserId || state.rollsLeft === 0}
